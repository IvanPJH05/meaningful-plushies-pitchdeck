...
function StoryVisual({ item, registerVideo, onVideoError }) {
  ...
  if (item.variant === "productCard") {
    return (
      <div className="product-card" aria-label="Creator match card">
        ...
        <div className="plush-stack">
          <span className="plush-dot pink" />
          <span className="plush-dot blue" />
          <span className="plush-dot gold" />
        </div>
  ...
  return (
    <div className="text-object">
      ...
      <strong>Creator Wave</strong>
    </div>
  );
}

export default function ScrollStory({ items = storyItems, height = "650vh" }) {
  ...
  useEffect(() => {
    ...
    const context = gsap.context(() => {
        ...
        scrollTrigger: {
          ...
          onUpdate: (self) => {
            ...
            sortedItems.forEach((item) => {
              ...
              if (node) {
                gsap.set(node, {
                  pointerEvents: inSegment ? "auto" : "none",
                  zIndex: inSegment ? 5 : 1
                });
              }
              ...
              if (item.type === "video") {
                ...
                if (video?.duration && Number.isFinite(video.duration)) {
                  videoTargets.current.set(item.id, localProgress * video.duration);
                }
              }
            });
      ...
      sortedItems.forEach((item) => {
        ...
        const enterDuration = segmentDuration * 0.58;
        const exitDuration = segmentDuration * 0.52;
        const exitStart = item.start + segmentDuration * 0.48;
        const fromVars = toGsapVars(item.animation.from);
        const focusVars = toGsapVars(item.animation.focus);
        const toVars = toGsapVars(item.animation.to);

        timeline.fromTo(
          node,
          {
            ...fromVars,
            scale: typeof fromVars.scale === "number" ? fromVars.scale * 0.985 : fromVars.scale
          },
          {
            ...focusVars,
            scale: typeof focusVars.scale === "number" ? focusVars.scale * 1.012 : focusVars.scale,
        ...
        timeline.to(
          ...
          {
            ...focusVars,
            duration: enterDuration * 0.18,
            ease: "sine.inOut"
          },
          item.start + enterDuration * 0.82
        );

        timeline.to(node, { ...toVars, duration: exitDuration, ease: "power2.in" }, exitStart);
      });
    }, sectionRef);
    ...
    return (
      <section className="scroll-story-fallback" aria-labelledby="story-title">
        <div className="section-heading">
          <p className="eyebrow">Creator acquisition story</p>
          <h2 id="story-title">A scroll-led path from creator discovery to plushie launch.</h2>
        </div>
        <div className="fallback-grid">
          {sortedItems.map((item) => (
            <article className="fallback-card" key={item.id}>
  ...
  return (
    <section
      ...
      style={{ "--story-height": height }}
      aria-labelledby="story-title"
    >
      <div className="sticky-story">
        <div className="story-background" aria-hidden="true" />
        <div className="story-copy-rail">
          <p className="eyebrow">Apple-style scroll story</p>
          <h2 id="story-title">Creator acquisition, told one plush moment at a time.</h2>
          <div className="story-progress-list" aria-label="Story progress">
            {sortedItems.map((item) => (
              <span className={activeId === item.id ? "is-active" : ""} key={item.id}>
                {item.kicker}
              </span>
            ))}
          </div>
        ...
        <div className="story-stage" aria-live="polite">
          {sortedItems.map((item) => (
            <article
              ref={(node) => registerObject(item.id, node)}
              className={`story-object story-object-${item.type}`}
              key={item.id}
            ...
            >
              <StoryVisual
                ...
                onVideoError={(id) => setVideoErrors((current) => ({ ...current, [id]: true }))}
              />
              <div className="object-copy">
                <span>{item.kicker}</span>
                <h3>{item.headline}</h3>
                <p>{videoErrors[item.id] ? item.fallbackBody || item.body : item.body}</p>
              </div>
            </article>
          ))}
        </div>
